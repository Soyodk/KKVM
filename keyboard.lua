...
function main()
	...
	while true do
		...
		if delta ~= 0 then table.insert(wheel, {delta, os.clock()+0.05}) end -- Фикс отображения прокрута колеса
	end
end
...
function imgui.OnDrawFrame()
	...
	if settings.v then
		...
		imgui.Begin(u8'Настройки клавиатуры и мыши', settings, imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize)
			imgui.PushItemWidth(140)
			imgui.Combo(u8'Тип клавиатуры', keyboard_type, {u8'Вся клавиатура', u8'Без NumPad', u8'Только цифры', u8'Компактные цифры', u8'Кнопки управления'})
			imgui.PopItemWidth()
			imgui.Checkbox(u8'Включить клавиатуру', keyboard)
			imgui.Checkbox(u8'Включить мышь', mouse)
			imgui.Checkbox(u8'Перемещение клавиатуры и мыши', move)
			imgui.PushItemWidth(140)
			if imgui.Combo(u8'Тема', theme, {u8'Зелёная', u8'Красная', u8'Пурпурная', u8'Фиолетовая', u8'Вишнёвая', u8'Жёлтая'}) then styles[theme.v]() end
			imgui.PopItemWidth()
	...
	if mouse.v then
		...
		imgui.Begin('##mouse', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.AlwaysAutoResize + (move.v and 0 or imgui.WindowFlags.NoMove) )
			...
			for key, v in ipairs(mouse_keys) do
				if key == 2 then renderWheel() imgui.SetCursorPosY(18) elseif key == 3 then imgui.SetCursorPosY(2) end -- Костыль
				local size = imgui.CalcTextSize(v[1])
				if isKeyDown(v[2]) then
					imgui.PushStyleColor(imgui.Col.ChildWindowBg, imgui.GetStyle().Colors[imgui.Col.ButtonActive])
				else
				...
				imgui.EndChild()
				imgui.PopStyleColor()
				if key ~= 3 then imgui.SameLine() end -- костыль
			end
		imgui.End()
		imgui.PopStyleColor()
		imgui.PopStyleVar()
